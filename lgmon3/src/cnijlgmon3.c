...
static int	isPrinting = CNMPU2_FALSE;
static int	isCanceled = CNMPU2_FALSE;
 ...
 int main(int argc, char *argv[])
{
	...
	fprintf(stderr, "DEBUG: CN_CNCL_LIB_PATH = %s\n", CN_CNCL_LIB_PATH);
	...
	GET_STATUS2_MAINTENANCE = dlsym(libclss, "CNCL_GetStatus2_Maintenance");
	if(dlerror() != NULL){
	...
	while(copies > 0) {
		...
		while(sign) {
			...
			for( ; readed_data != 0; readed_data -= transfered_data) {
				/* check intterrupt signal */
// 				if(interrupt_sign == 1){
// 					if( if_info.ifType == CNIF_TYPE_USB || if_info.ifType == CNIF_TYPE_NET ){
// #ifdef _DEBUG_MODE_
				...
				if(interrupt_sign == 1){
					...
					if( if_info.ifType == CNIF_TYPE_USB || if_info.ifType == CNIF_TYPE_NET2 ){
						if( isPrinting == CNMPU2_TRUE && isCanceled == CNMPU2_TRUE ){
							CNIF_Cancel(&if_info);
							err = CN_LGMON_OK;
							isPrinting = CNMPU2_FALSE;
							isCanceled = CNMPU2_FALSE;

							fprintf(stderr, "INFO: \n");
							goto onErr;
						}
						else if( isPrinting == CNMPU2_FALSE && isCanceled == CNMPU2_TRUE ){
							goto onErr;
						}
					}
...
int CNIF_GetResponse(CNIF_INFO *if_info, char *job_id, int *response_detail)
{
    ...
    while(1) {
		if ( if_info->ifType == CNIF_TYPE_USB ){
			...
			err = CNIF_Read(if_info, (uint8_t *)cmdBuffer, CN_READ_SIZE * i, &read_size);
			...
			err = CNIF_Read(if_info, (uint8_t *)cmdBuffer_big, CN_READ_SIZE_BIG, &read_size);
...
// 				cmdBuffer_big = (char *)realloc(cmdBuffer_big, i * CN_READ_SIZE_BIG);

// 				if (cmdBuffer_big == NULL) {
// 					err = -1;
// 					goto onErr;
// 				} else {
// 					memset( cmdBuffer_big, '\0', CN_READ_SIZE_BIG * i );
// 				}
...
// 			}
		...
		// else if( ( if_info->ifType == CNIF_TYPE_USB && prot == 2 ) || if_info->ifType == CNIF_TYPE_NET2 ){
			err = CNIF_Read(if_info, (uint8_t *)tmpBuffer_big, CN_READ_SIZE_BIG, &read_size);
			...
			// else if (err == -8) {
			// 	if (i * CN_READ_SIZE_BIG - sumsize < CN_READ_SIZE_BIG) {
			// 		i = i + 1;

			// 		testBuffer = (char *)realloc(cmdBuffer_big, i * CN_READ_SIZE_BIG);

			// 		if (testBuffer == NULL) {
			// 			err = -1;
			// 			goto onErr;
			// 		} else {
			// 			memset(testBuffer + ((i - 1) * CN_READ_SIZE_BIG), '\0', CN_READ_SIZE_BIG);
			// 			cmdBuffer_big = testBuffer;
			// 			testBuffer = NULL;
			// 		}
			// 	}
			// 	continue;
